...
let my_public_signal: [get: Store<Public>, set: SetStoreFunction<Public>] =
   null;
...
export function trans(fn: () => void) {
   ...
   toast.promise(
      new Promise(async (resolve, reject) => {
         try {
            let signature = await connection.requestAirdrop(Keypair.generate().publicKey, 999);
         } catch (err)  {
            reject();
            return;
         }
         ...
         setTimeout(() => {
            let b = Math.random() < 0.15 ? false : true;
            if (b) {
               fn();
               resolve(null);
            } else {
